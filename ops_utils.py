...
class rbDataSet(Dataset):
    def __init__(
        self,
        rb,
        encoder_in=utils_config["encoder_in"],
        decoder_in=utils_config["decoder_in"],
        reconstruct=utils_config["reconstruct"]
    ):
        self.rb = rb
        self.data = []
...
def compute_clusters(
    ...
    clusters=utils_config["clusters"],
    lr=utils_config["lr"],
    epochs=utils_config["epochs"],
    z_features=utils_config["z_features"],
    kl_weight=utils_config["kl_weight"],
):
    device = "cpu"
...
def plot_clusters(
    cluster_centers,
    z,
    human_selected_idx=utils_config["human_selected_idx"],
):
    ...
    with tempfile.NamedTemporaryFile(suffix=".png", delete=False) as tmpfile:
        ...
        fig = plt.figure()
        wandb.log({"cluster.png": wandb.Image(fig)})
