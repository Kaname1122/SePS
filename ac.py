...
from ops_utils import compute_clusters, utils_config
...
config = {
    ...
    "total_steps": int(25e5),  # total_steps * n_steps * parallel_envs = 5e7
    "log_interval": int(2e3),  # 2000
...
run = wandb.init(
    project="lab",
    name="SePS_foraging_5e7",
    config=config,
)


class Torcherize(VecEnvWrapper):
    def __init__(self, venv, algorithm_mode=config["algorithm_mode"]):
        super().__init__(venv)
    ...
    def reset(self, device=config["device"], parallel_envs=config["parallel_envs"]):
        obs = self.venv.reset()
    ...
    def step_wait(self, device=config["device"], parallel_envs=config["parallel_envs"]):
...
def _compute_returns(storage, next_value, gamma=config["gamma"]):
    returns = [next_value]
...
def _log_progress(
    ...
    logging.info(f"Updates {step}, Environment timesteps {parallel_envs* n_steps * step}")
    logging.info(
        f"UPS: {ups:.1f}, FPS: {fps:.1f}, ({100*step/total_steps:.2f}% completed)"
    )

    logging.info(f"Last {len(infos)} episodes with mean reward: {mean_reward:.3f}")
    logging.info(f"Battles won: {battles_won:.1f}%")
    logging.info("-------------------------------------------")

    squashed_info = _squash_info(infos)
    squashed_info["environment_steps"] = parallel_envs * n_steps * step
    wandb.log(squashed_info)
...
def _compute_loss(
    ...
    wandb.log(
        {
            "model/actor_loss": actor_loss.item(),
            "model/value_loss": value_loss.item(),
...
def main(
    ...
    parallel_envs=config["parallel_envs"],
    n_steps=config["n_steps"],
    use_proper_termination=config["use_proper_termination"],
    central_v=config["central_v"],
    ...
    for step in range(total_steps):
        ...
        if algorithm_mode == "ops" and step in [ops["delay"] + ops["pretraining_steps"]*(i+1) for i in range(ops["pretraining_times"])]:
            ...
            logging.info(f"model.laac_sample: {model.laac_sample}")
            # wandb
            columns = [f"agent_{i}" for i in range(len(cluster_idx.tolist()))]
            wandb.log({"cluster_idx": wandb.Table(columns=columns, data=[cluster_idx.tolist()])})
